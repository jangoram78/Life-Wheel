...
(function (window) {
  ...
  var defaultDomains = [
    ...
    {
      name: "Work & Purpose",
      sub: ["Career", "Mastery", "Finance", "Legacy"],
    },
    {
      name: "Social",
      sub: ["Family", "Friends", "Colleagues", "Civic Contribution"],
    },
    {
      name: "Emotional",
      sub: ["Emotional Stability & Regulation", "Joy", "Identity"],
    },
  ...
  function ensureCurrentFrames() {
    ...
    if (
      ...
      typeof userState.monthlyScores !== "object"
    ) {
      userState.monthlyScores = {};
    }
    ...
    for (var dIdx = 0; dIdx < userState.domains.length; dIdx++) {
      ...
      for (var sIdx = 0; sIdx < userState.domains[dIdx].sub.length; sIdx++) {
        if (
          typeof userState.monthlyScores[monthKey][dIdx][sIdx] !== "number"
        ) {
  ...
  // ------- One-time seeding from activities.js -------
  function hasAnyTemplates() {
    var t = userState.templates || {};
    for (var d in t) {
      if (!t.hasOwnProperty(d)) continue;
      var subMap = t[d];
      for (var s in subMap) {
        if (!subMap.hasOwnProperty(s)) continue;
        var slot = subMap[s];
        if (
          (slot.micro && slot.micro.length) ||
          (slot.standard && slot.standard.length) ||
          (slot.deep && slot.deep.length)
        ) {
          return true;
        }
      }
  ...
  function seedTemplatesFromExternalActivities() {
    if (hasAnyTemplates()) return; // already seeded or user-defined

    var external =
      window.LW_ACTIVITIES || window.ACTIVITIES || window.MASTER_TASKS || null;
    if (!external || !Array.isArray(external)) return;

    // Build name->index maps
    var domIndex = {};
    userState.domains.forEach(function (d, idx) {
      domIndex[d.name] = idx;
    });

    var i;
    for (i = 0; i < userState.domains.length; i++) {
      var dom = userState.domains[i];
      dom.subIndex = {};
      dom.sub.forEach(function (sName, sIdx) {
        dom.subIndex[sName] = sIdx;
      });
    }

    external.forEach(function (line) {
      if (typeof line !== "string") return;
      // Expect: Domain|Sub|Level|Label
      var parts = line.split("|");
      if (parts.length < 4) return;
      var dName = parts[0].trim();
      var sName = parts[1].trim();
      var level = parts[2].trim().toLowerCase();
      var label = parts.slice(3).join("|").trim(); // in case '|' appears in text

      var dIdx = domIndex[dName];
      if (typeof dIdx !== "number") return;
      var dom = userState.domains[dIdx];
      var sIdx = dom.subIndex[sName];
      if (typeof sIdx !== "number") return;
      if (!["micro", "standard", "deep"].includes(level)) return;

      var slot = ensureTemplateSlot(dIdx, sIdx);
      if (!Array.isArray(slot[level])) slot[level] = [];
      if (slot[level].indexOf(label) === -1) {
        slot[level].push(label);
      }
    });
  }
  ...
  function buildNudge() {
    ...
    if (type === "soft") {
      msg =
        "You’re broadly on track. If you can, do one small thing today that supports " +
        worst.name +
        ".";
    } else if (type === "standard") {
      msg =
        worst.name +
        " is falling behind this week. Protect 5–10 minutes today to nudge it forward.";
    } else {
      msg =
        "You’ve been neglecting " +
        worst.name +
        ". Ring-fence one simple action today so this domain doesn’t keep sliding.";
    }
  ...
  function generateBaseTaskPack() {
    ...
    var neglected = domainInfo.filter(function (d) {
      return d.neglect !== "none";
    });
    ...
    for (var i = 0; i < focus.length; i++) {
      ...
      if (templ) {
        ...
        label =
          "Do one small 5–10 min action for " + d.name + " (your choice).";
      }
    ...
    if (focus.length) {
      ...
      if (templStd) {
        ...
        labelStd =
          "Spend 15–20 mins deliberately improving " +
          worst.name +
          " in a concrete way.";
      }
    ...
    if (momentum >= 0.3 && domainInfo.length) {
      ...
      if (templDeep) {
        ...
        labelDeep =
          "Optional 20–30 min deep block for " +
          best.name +
  ...
  function generateTaskForDomainAndDifficulty(domainIdx, difficulty) {
    var todayKey = userState.todayKey;
    var domName =
      (userState.domains[domainIdx] &&
        userState.domains[domainIdx].name) ||
      "Domain";
  ...
  function applyTaskCompletionImpact(task) {
    ...
    var current =
      typeof userState.weeklyScores[weekKey][idx] === "number"
  ...
  async function init() {
    ...
    if (stored && typeof stored === "object") {
      ...
      if (
        !userState.domains ||
        !Array.isArray(userState.domains) ||
        !userState.domains.length
      ) {
    ...
    ensureCurrentFrames();
    seedTemplatesFromExternalActivities();
  ...
  function getTodayVM() {
    ...
    for (i = 0; i < domains.length; i++) {
      ...
      domainInfo.push({
        ...
        neglectRank: neglectRank,
        lastWeek:
          prevWeek && typeof prevWeek[i] === "number" ? prevWeek[i] : null,
      });
    }
  ...
  function getMonthlyVM() {
    ...
    for (var i = 0; i < userState.domains.length; i++) {
      ...
      for (var sIdx = 0; sIdx < dom.sub.length; sIdx++) {
        var v =
          mk[i] && typeof mk[i][sIdx] === "number" ? mk[i][sIdx] : 0;
  ...
  async function completeTask(taskId) {
    ...
    var replacement = generateTaskForDomainAndDifficulty(
      found.domainIdx,
      found.difficulty
    );
  ...
  async function updateDomainSubdomains(domainIdx, newSubs) {
    ...
    for (var sIdx = 0; sIdx < newSubs.length; sIdx++) {
      if (
        typeof userState.monthlyScores[monthKey][domainIdx][sIdx] !== "number"
      ) {
